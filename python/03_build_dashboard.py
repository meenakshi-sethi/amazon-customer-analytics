...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* taxonomy strip — the kinds of statistical analysis */
  .taxstrip{display:flex;flex-wrap:wrap;gap:7px;margin:14px 0 4px}
  .taxchip{font-size:11px;letter-spacing:.05em;padding:5px 12px;border:1px solid var(--rule);
    border-radius:999px;color:var(--muted);background:#fff;text-transform:uppercase}
  .taxchip.used{border-color:var(--ink);color:var(--ink);font-weight:600}
  .taxchip .q{display:block;font-size:9.5px;letter-spacing:.03em;text-transform:none;
...
<div class="sheet">
  ...
  <section class="chap" id="c5">
    ...
    <div class="taxstrip">
      <span class="taxchip used tip" data-tip="<b>Descriptive analysis.</b> Summarizes what happened &mdash; averages, ranges, charts. No verdicts, just a clear picture. Used here: chapters I&ndash;IV of this report.">Descriptive<span class="q">what happened?</span></span>
      <span class="taxchip used tip" data-tip="<b>Exploratory data analysis (EDA).</b> Look before you leap &mdash; plots and summaries to find patterns before any hypothesis is fixed. Used here: the exploration notebook that ships with this report.">Exploratory<span class="q">what's in there?</span></span>
      <span class="taxchip used tip" data-tip="<b>Diagnostic analysis.</b> Asks why something happened by examining relationships between variables. Used here: the sentiment cross-tab (Ch. IV) explaining hidden detractors &mdash; low stars, warm words.">Diagnostic<span class="q">why did it happen?</span></span>
      <span class="taxchip used tip" data-tip="<b>Inferential analysis.</b> Does the pattern hold beyond this sample? Hypothesis tests, p-values, effect sizes. Used here: this entire chapter &mdash; every verdict card below.">Inferential<span class="q">does it hold up?</span></span>
      <span class="taxchip tip" data-tip="<b>Predictive analysis.</b> Forecasts what might happen next from historical data &mdash; time series, ML models. Not used: the retention timeline (Ch. II) only describes the past; no forecasting was in scope.">Predictive<span class="q">what's next?</span></span>
      <span class="taxchip tip" data-tip="<b>Prescriptive analysis.</b> Recommends what to do next, combining data with business rules. Not formally used &mdash; though the advocate shortlist (Ch. VI) leans this way.">Prescriptive<span class="q">what to do?</span></span>
      <span class="taxchip tip" data-tip="<b>Causal analysis.</b> Proves X causes Y &mdash; needs experiments (A/B tests), not observation. Not used: every relationship below is an association, not a proven cause. The power analysis sizes the experiment that could change that.">Causal<span class="q">did X cause Y?</span></span>
      <span class="taxchip tip" data-tip="<b>Bayesian analysis.</b> Updates the probability of a belief as evidence arrives. Not used: every test in this chapter is the classic (frequentist) kind.">Bayesian<span class="q">update beliefs</span></span>
    </div>
    <p class="body" style="font-size:12px;color:var(--muted);margin:6px 0 0">The dark chips are the kinds of analysis this report actually performs; hover any chip to learn what that kind is for. The light ones are deliberately out of scope &mdash; a report that claims more than it proves is lying.</p>
    ...
    <div class="chooser">
      ...
      <p class="chintro">Every test in this chapter was picked by one rule: <b>match the tool to the shape of the data and the question.</b> The selection path, in order: define the objective &rarr; identify the data type (category or quantity) &rarr; count the variables involved &rarr; check each method's assumptions &rarr; ask whether the observations are paired or independent. Categories (labels like "Champion" or "Positive") get the counting test; quantities (votes, days, ratings) get the relationship tests; lopsided quantities get the rank-based versions that stay honest.</p>
      <table>
        ...
        <tr><td>Do three or more groups differ?</td><td>Quantities, well-balanced</td><td>ANOVA (averages)</td><td>&mdash; (same skew problem)</td></tr>
        <tr><td>Do three or more groups differ?</td><td>Quantities, lopsided</td><td>Kruskal-Wallis (rankings)</td><td>&mdash; (every question here compares two)</td></tr>
...
(function(){
  ...
  if(co && co.variables && co.variables.length){
    ...
    window.__renderCorr=function(){
      ...
      if(window.__wireTips) window.__wireTips(document.getElementById('corrBody'));
    };
  ...
  /* ---------- skewness exhibit (figures rendered by the Python pipeline; tabbed) ---------- */
  const ex=document.getElementById('skewex');
  if(ex && DATA.skewfigs && DATA.skewfigs.length){
    window.__skewIdx=0;
    window.__renderSkew=function(){
      const i=window.__skewIdx, f=DATA.skewfigs[i];
      ex.innerHTML=`<div class="chooser" style="margin-top:20px">
        ...
        <p class="chintro">These three shapes are the reason this chapter reaches for rank-based tests: a t-test assumes a bell curve, and none of these is one. The figures are drawn by the Python statistics stage (matplotlib) from the cleaned data &mdash; this report only displays them; the exploration notebook regenerates them live. One at a time, to keep the page clean &mdash; pick a distribution:</p>
        <div class="corrbtns" style="margin:12px 0 0">
          ${DATA.skewfigs.map((g,j)=>`<button class="${j===i?'on':''}" onclick="__skewTab(${j})">${g.title}</button>`).join('')}
        </div>
      ...
      if(window.__wireTips) window.__wireTips(ex);
    };
    window.__skewTab=function(i){window.__skewIdx=i;window.__renderSkew();};
    window.__renderSkew();
  }

  /* ---------- hover-to-learn: wire every data-tip to the shared tooltip ----------
     Re-runnable: renderers that replace DOM with new data-tip elements
     (correlation toggle, skew tabs) call __wireTips again on their container. */
  window.__wireTips=function(root){
    (root||document).querySelectorAll('[data-tip]').forEach(el=>{
      if(el.__tipped) return; el.__tipped=true;
